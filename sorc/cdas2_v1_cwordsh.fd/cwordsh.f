...
C   PRGMMR: KEYSER           ORG: NP22        DATE: 2012-10-13
C
...
C$$$
      ...
      call w3tagb('CWORDSH',2012,0287,0332,'NP22')
...
c     Open the input and output files.
      ...
      if(cword.eq.'block') then
         print '(" blocking from: ",A)', ufile
         print '("            to: ",A)', bfile
         call cobfl(ufile,'r')
         open(51,file=bfile,form='unformatted')
      else
         print '(" unblocking from: ",A)', bfile
         print '("              to: ",A)', ufile
         call cobfl(bfile,'r')
         call cobfl(ufile,'w')
      endif
...
c     Close the input and output files.
   ...
   88 continue
      ...
      if(cword.eq.'block') then
         if(igo.eq.5) then
            print '(" ***WARNING: CWORDSH - No input BUFR messages ",
     .       "were blocked into output file - no output file created")'
         else if(istop.eq.4) then
            print '(" ***WARNING: CWORDSH - One or more input BUFR ",
