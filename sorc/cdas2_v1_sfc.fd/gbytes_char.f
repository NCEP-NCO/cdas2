      ...
      character*1 in(*)
      integer iout(*)
c      write(*,*) 'gbytec >> gbytesc iskip,nbyte=', iskip, nbyte
      CALL GBYTESC(IN,IOUT,ISKIP,NBYTE,0,1)
c      write(*,*) 'gbytec << gbytesc'
      RETURN
      END
...
c      write(*,*) 'gbytec: iskip,nbytes, nskip,n =',iskip,nbyte,nskip,n
...
c        get data from last byte
     ...
     1		iand(ishft(mova2i(in(index)),
     2		-(8-bitcnt)),ones(bitcnt)))
         endif
