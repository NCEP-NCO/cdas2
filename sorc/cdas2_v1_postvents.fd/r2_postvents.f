...
c 8/2014 subroutine fits: limit N to 32867 so can pack
c change BMISS from 10e10 to 10e8 - WCOSS change
C  doesn't appear to be necessary but good so real -> int can be done
...
C     limit stat(1) to 32767 so that packing does not overflow

      if (STAT(1).lt.32767) then
          STAT(1) = STAT(1) + 1.
          STAT(2) = STAT(2) + FIT
          STAT(3) = STAT(3) + FIT**2
      endif
